...
public class HDFSWCApp02 {
    ...
    public static void main(String[] args) throws Exception{
        ...
        HDFSMapper mapper = (HDFSMapper)aClass.newInstance();

        HDFSContext context = new HDFSContext();

        // 1.读
        Path path = new Path(properties.getProperty(Constants.INPUT_PAHT));
        ...
        Path outputPath = new Path(properties.getProperty(Constants.OUTPUT_PAHT));
        FSDataOutputStream out = fileSystem.create(new Path(outputPath, new Path(properties.getProperty(Constants.OUTPUT_FILE))));
